      ...
      <section className="panel">
        ...
        <ul>
          {lesson.officialDocs.map((doc) => (
            <li key={doc.url}><a href={doc.url} target="_blank" rel="noreferrer">{doc.title}</a></li>
          ))}
        </ul>
      </section>
      <div className="lesson-actions">
        <Link className="primary-button" to={`/quest/${lesson.slug}/practice`}>실습 시작하기</Link>
      </div>
      <section className="quiz-list">
        {quizzes.map((quiz) => <QuizCard key={quiz.id} quiz={quiz} onComplete={completeQuiz} />)}
      </section>
    </div>
  )
}
